...
class NodeFormerConv(gnn.MessagePassing):
    ...
    def forward(self, z, adjs, tau, sat=0):
        ...
        out = self.propagate(adjs[1], v=v, qk=qk, edge_attr=None, size=None,
                                 return_attn=False)
        z_next = rearrange(out, 'n h d -> n (h d)')
...
class dag_nodeformer(nn.Module):
    '''
    ...
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers=2, num_heads=4, dropout=0.0,
        ...
        super(dag_nodeformer, self).__init__()
